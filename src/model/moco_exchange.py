...
class MoCo_Model_Exchange(nn.Module):
    def __init__(self, args, queue_size=65536, momentum=0.999, temperature=0.07):
        ...
        '''
        super(MoCo_Model_Exchange, self).__init__()
    ...
    def InfoNCE_logits(self, f_q, f_k):
        ...
        neg_change = torch.rand(neg.size()[0],neg.size()[1]-1)
        a = torch.argmax(neg, 1)
        for i in range(neg.size()[0]):
            for j in range(neg.size()[1]-1):
                if j < a[i]:
                    neg_change[i][j] = neg[i][j]
                else:
                    neg_change[i][j] = neg[i][j+1]


        logits = torch.cat((pos, neg_change), dim=1)
